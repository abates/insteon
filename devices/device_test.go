...
func TestOpen(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:     "basic",
			ackErr:   nil,
			acks:     []*insteon.Message{{Command: commands.Command(0x01)}},
			read:     []*insteon.Message{{Dst: insteon.Address(uint32(insteon.SwitchDomain)<<16 | 1<<8 | 59), Command: commands.SetButtonPressedResponder}},
		...
		},
		{
			name:     "unlinked",
			ackErr:   ErrNak,
			acks:     []*insteon.Message{{Flags: insteon.StandardDirectNak, Command: commands.Command(0x00ff)}},
			read:     []*insteon.Message{{Dst: insteon.Address(uint32(insteon.SwitchDomain)<<16 | 1<<8 | 59), Command: commands.SetButtonPressedResponder}},
